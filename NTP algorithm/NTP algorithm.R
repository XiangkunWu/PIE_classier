...
n.marker <- 30 # find 30 markers for each subtype

# 1. load data (if you provide subtype on your own, please make sure you have the same format of 'CNMF_subtype' file)
train.exp <- read.table("TCGAmatrix.txt", sep = "\t", row.names = 1, check.names = F, stringsAsFactors = F, header = T)#colunm: sample; row:genes
test.exp <- read.table("E-MTAB-1980_matrix.txt", sep = "\t", row.names = 1, check.names = F, stringsAsFactors = F, header = T)#colunm: sample; row:genes
